...
def createSATBDataset(model_config):
	...
	dsd_train = glob.glob(model_config['satb_path_train']+"/**/*.wav",recursive=True)                                                              # Get entirety of training set
...
def SATBBatchGenerator(hdf5_filepath, batch_size, num_frames, use_case=0, partition='train', resampling_fs=22050, debug=False):
    ...
    while True:
        ...
        for i in range(batch_size):
            ...
            out_shapes['alto'][i]    = (out_shapes['tenor'][i]/scaler)
            out_shapes['tenor'][i]   = (out_shapes['alto'][i]/scaler)
